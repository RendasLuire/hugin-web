...
export default function TreasuryCard({
  ...
  const goaldRatio = goald > 0 ? (balance / goald) * 100 : 1;
  
  let auraClass = 'border-zinc-700';
  let runicGlow = '';
  let currentIcon = "/icons/yggdrasil-280x280.png";
  ...
  return (
    ...
    className={`relative rounded-xl overflow-hidden p-4 transition-transform hover:scale-[1.015]
      ...
      {runesOpacity > 0  && (
        ...
        <Image src={currentIcon} alt="icono" width={100} height={100} />
        </div>
        ...
        {/* Barra de esencia */}
        {
          ...
          style={{
            width: `${essenceRatio}%`,
            background: `linear-gradient(90deg, var(--color-muted) 0%,var(--color-runic) 100%)`,
            boxShadow: `0 0 10px var(--color-runic),
            0 0 20px var(--color-muted)`,
          }}
          >
          ...
          >
          <span className="z-10">{Math.round(goaldRatio)}%</span>
          </div>
          </div>
          </div>
              }
              <p className="text-xs text-center text-zinc-400 mb-1 font-light italic">{balance}</p>
            </div>
          }
        </div>
        </div>
